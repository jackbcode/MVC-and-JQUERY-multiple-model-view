...
namespace PolicyCheck.Helper
{
    ...
    public class RiskQuoteBuilder
    {
        public static void BuildQuotes(string clientRef)
        {
            ParagonEntities15 UploadPolicy = new ParagonEntities15();

            IEnumerable policyUpload = UploadPolicy.PolicyCheckUploadParagon(clientRef);

            var NumberOfPeople = UploadPolicy.PolicyCheckUploadParagon(clientRef).Select(x => x.Propforenames).Count();

            var policyUploadDS = UploadPolicy.PolicyCheckUploadParagon(clientRef).ToList();

            IEnumerable SpecItems = UploadPolicy.SpecItemUploadParagon(clientRef);

            var SpecItemsList = UploadPolicy.SpecItemUploadParagon(clientRef).ToList();

            var NumberOfSpecItems = UploadPolicy.SpecItemUploadParagon(clientRef).Select(x => x.amount).Count();
            ...
            var ClaimsuploadDS = UploadPolicy.ClaimsUploadParagon(clientRef).ToList();

            var UploadProposer = UploadPolicy.PCProposersParagon1(clientRef).ToList();
            ...
            foreach (var p in UploadProposer)
            {
                ...
                abiCode12.Value = NumberOfClaims == 0 ? "N" : "Y";
